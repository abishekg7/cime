...
#include <mpif.h>
  ...
  !--------------------------------------------------------------------------
   ...
   character(*), parameter :: F0R1 = "('"//subname//" : ', A, 2g23.15,A )"
...
contains
  ...
  subroutine component_init_cc(Eclock, comp, comp_init, infodata, NLFilename, &
    ...
    if (iamroot_CPLID .and. comp(1)%present) then
       call t_stampf(wall2, usr, sys)
       write(logunit,F0R1) 'component '//trim(comp(1)%ntype)//' initialization complete in ', (wall2-wall1)/60.0, ' minutes.'
       call shr_sys_flush(logunit)
    endif
